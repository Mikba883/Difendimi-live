...
interface AuthDialogEnhancedProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onAuthSuccess: () => void;
}

export function AuthDialogEnhanced({ open, onOpenChange, onAuthSuccess }: AuthDialogEnhancedProps) {
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      console.log('Auth state changed in dialog:', event, session?.user?.email);
  ...
  }, [open, onAuthSuccess, onOpenChange, trackEvent]);

  const handleGoogleSignIn = async () => {
    setLoading(true);
    console.log('Starting Google OAuth flow from dialog...');
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      setLoading(false);
      // Email verification flow will handle the rest
    }
  };
              ...
              <TabsContent value="signup">
                <form onSubmit={handleSignUp} className="space-y-4">
                  ...
                  <Button 
                    ...
                    className="w-full h-11 bg-white hover:bg-gray-50 text-gray-900 border border-gray-300 font-medium"
                    disabled={loading}
                  >
                    {loading ? (
                  ...
                  <div className="space-y-2">
                    <div className="flex items-start space-x-2">
                      ...
                      <label
                        htmlFor="privacy"
                        className={`text-xs cursor-pointer select-none ${consentError ? 'text-destructive' : 'text-muted-foreground'}`}
                      >
                        Procedendo dichiaro di aver letto e accettato i{" "}
